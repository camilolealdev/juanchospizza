...
function loadCertificate() {
  ...
  // Warn on approaching expiration (2026-07-21 hardening). La DIAN rechaza
  // firmadas con cert expirado (>0 días, no >N días) y el error es opaco al
  // operador ("rechazado por la DIAN, revisar firma"). Si quedan < 30 días
  // imprimimos un warning explícito al boot para que el operador planee
  // rotación antes de que un envío a producción falle.
  const notAfter = certBag.cert?.validity?.notAfter;
  if (notAfter instanceof Date) {
    const daysRemaining = (notAfter.getTime() - Date.now()) / (24 * 3600 * 1000);
    if (daysRemaining < 30) {
      console.warn(
        `[DIAN] Certificado de firma expira en ${daysRemaining.toFixed(1)} días ` +
          `(notAfter=${notAfter.toISOString()}). Renovar ANTES del próximo envío ` +
          'a producción — la DIAN rechaza firmadas con cert expirado.'
      );
    }
  }
...
export function calcularCUFE(invoiceData) {
  ...
  // Field-level validation (2026-07-21 hardening). Antes calcularCUFE
  // aceptaba un invoiceData parcial y producía un SHA-384 sobre campos
  // vacíos — la DIAN rechazaba luego como "mismatch" sin pista del
  // campo faltante. Ahora fallamos rápido con un error local explícito.
  const requiredTop = ['invoiceNumber', 'fechaEmision', 'emisorInfo', 'receptorInfo', 'totales'];
  for (const k of requiredTop) {
    if (!invoiceData[k]) {
      throw new Error(`[DIAN] calcularCUFE: campo requerido faltante: ${k}`);
    }
  }
  if (!invoiceData.emisorInfo.nit) {
    throw new Error('[DIAN] calcularCUFE: emisorInfo.nit requerido (incluso si es placeholder)');
  }
  if (!invoiceData.receptorInfo.numeroIdentificacion) {
    throw new Error('[DIAN] calcularCUFE: receptorInfo.numeroIdentificacion requerido');
  }
  // totales.subtotal/iva/total pueden ser 0 legítimamente — verificar
  // explícitamente con !== undefined para distinguir ausente de cero.
  for (const k of ['subtotal', 'iva', 'total']) {
    if (invoiceData.totales[k] === undefined || invoiceData.totales[k] === null) {
      throw new Error(`[DIAN] calcularCUFE: totales.${k} requerido (incluso si es 0)`);
    }
  }
