...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      // batch-status no recibe locationId -- solo ids. Sin este chequeo un
      // OPERATOR podía pasar ids de mesas de OTRA sede y mutarlas igual.
      if (req.auth.role !== 'ADMIN' && req.auth.locationId) {
        const ownerCheck = await pool.query(
          'SELECT id FROM dining_tables WHERE id = ANY($1::text[]) AND "locationId" != $2',
          [ids, req.auth.locationId]
        );
