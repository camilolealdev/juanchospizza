  ...
  async (req, res) => {
    try {
      ...
      if (email) {
        ...
        }).catch((err) => logger.error({ err }, '[Email] Error enviando bienvenida'));
      }
    } catch (e) {
      if (e.code === '23505') {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (vip !== undefined) {
        params.push(vip);
        updates.push(`vip = $${params.length}`);
      }
      if (notas !== undefined) {
        params.push(notas);
  ...
  async (req, res) => {
    try {
      ...
      if (direccion !== undefined) {
        ...
        updates.push(`direccion = $${params.length}`);
      }
      if (notas !== undefined) {
        params.push(notas);
        updates.push(`notas = $${params.length}`);
      }
      if (cumpleanos !== undefined) {
        params.push(cumpleanos);
        updates.push(`cumpleanos = $${params.length}`);
      }
...
router.delete('/api/clients/:id', authMiddleware, requireRole('ADMIN', 'MARKETING'), async (req, res) => {
  try {
    ...
    // FK desde orders.clientId / push_subscriptions.clientId sin CASCADE a
    // propósito -- preserva historial de pedidos. Un cliente con pedidos
    // reales no debe poder borrarse, solo marcarse "perdido" vía PATCH.
    if (e.code === '23503') {
      return res.status(409).json({
        error: 'No se puede eliminar: el cliente tiene pedidos asociados. Márcalo como "perdido" en su lugar.',
      });
    }
...
router.get('/api/clients/:id/orders', authMiddleware, requireRole('ADMIN', 'MARKETING'), async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT * FROM orders WHERE "clientId" = $1 ORDER BY "createdAt" DESC LIMIT 20`, [
      req.params.id,
    ]);
