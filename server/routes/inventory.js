...
router.get('/api/inventory', authMiddleware, requireRole('ADMIN', 'OPERATOR'), async (req, res) => {
  try {
    // activo=false (dado de baja vía PUT) se oculta por defecto -- antes
    // esta query nunca filtraba por activo, así que un ítem "eliminado"
    // seguía apareciendo en el listado para siempre. ?includeInactive=true
    // lo trae de vuelta para auditoría/reactivación.
    const query =
      req.query.includeInactive === 'true'
        ? 'SELECT * FROM inventory_items ORDER BY nombre'
