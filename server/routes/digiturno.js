      ...
      try {
        ...
        setImmediate(() => {
          try {
            ...
            logger.error({ err }, '[WS] Error notificando nuevo ticket');
          }
        });
    ...
    try {
      ...
      setImmediate(() => {
        try {
          ...
          logger.error({ err }, '[WS] Error notificando cambio de ticket');
        }
      });
    } catch (_e) {
  ...
  try {
    ...
    setImmediate(() => {
      try {
        ...
        logger.error({ err }, '[WS] Error notificando eliminación de ticket');
      }
    });
  } catch (_e) {
