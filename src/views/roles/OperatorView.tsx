        ...
        <div className="space-y-4">
          {['Preparación', 'Horno', 'Empaque', 'Reparto'].map((s, i) => (
            <button
              ...
              onClick={() => {
                setStation(s);
                showToast(`Estación: ${s}`);
              }}
              className={`w-full text-left p-4 rounded-xl transition-all ${station === s ? 'bg-orange-600 font-bold text-white' : 'hover:bg-stone-800 text-stone-400'}`}
            >
              {s}
            </button>
          ))}
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6 p-4 md:p-8 overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {pending.map((order) => (
            <div
              key={order.id}
              className="bg-stone-900 p-6 rounded-2xl border-l-4 border-l-stone-600 border border-stone-800 shadow-lg space-y-4"
            >
              ...
              <button
                onClick={() =>
                  updateStatus(order.id, OrderStatus.PREPARING, `Preparación iniciada para ${order.orderNumber}`)
                }
                className="w-full bg-stone-800 hover:bg-orange-600 py-3 rounded-lg text-xs font-bold transition-all"
              >
                INICIAR PREPARACIÓN
              </button>
            </div>
        ...
        <div className="space-y-6">
          ...
          {preparing.map((order) => (
            <div
              key={order.id}
              className="bg-stone-900 p-6 rounded-2xl border-l-4 border-l-orange-600 border border-stone-800 shadow-lg space-y-4"
            >
              <div className="flex justify-between">
              ...
              <button
                onClick={() => updateStatus(order.id, OrderStatus.READY, `Pedido ${order.orderNumber} terminado`)}
                className="w-full bg-orange-600 hover:bg-orange-500 py-3 rounded-lg text-xs font-bold transition-all"
              >
                TERMINAR
              </button>
            </div>
        ...
        <div className="space-y-6">
          ...
          {ready.map((order) => (
            <div
              key={order.id}
              className="bg-stone-900 p-6 rounded-2xl border-l-4 border-l-green-600 border border-stone-800 shadow-lg space-y-4"
            >
              <div className="flex justify-between">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => updateStatus(order.id, OrderStatus.COMPLETED, 'Entrega local registrada')}
                  className="flex-1 bg-green-600 hover:bg-green-500 py-3 rounded-lg text-[10px] font-black tracking-widest transition-all"
                >
                  ENTREGA LOCAL
                </button>
                <button
                  onClick={() => updateStatus(order.id, OrderStatus.ASSIGNED, 'Domicilio asignado a repartidor')}
                  className="flex-1 bg-stone-800 hover:bg-blue-600 py-3 rounded-lg text-[10px] font-black tracking-widest transition-all"
                >
                  DOMICILIO
                </button>
              </div>
