  ...
  const getStatusColor = (status: OrderStatus) => {
    switch (status) {
      case OrderStatus.PENDING:
        return 'border-stone-600 bg-stone-900/30';
      case OrderStatus.CONFIRMED:
        return 'border-blue-600 bg-blue-900/10';
      case OrderStatus.PREPARING:
        return 'border-orange-600 bg-orange-900/10';
    ...
    <div className="flex flex-col md:flex-row h-screen bg-stone-950 text-white overflow-hidden">
      <div className="w-full md:w-72 bg-stone-900 border-b md:border-r border-stone-800 p-4 md:p-8 flex flex-col shrink-0">
        ...
        <div className="space-y-6 flex-1">
          ...
          <button
            ...
            className="w-full py-3 bg-stone-950 border border-white/10 rounded-xl text-[10px] font-black uppercase hover:bg-stone-800 transition-all"
          >
            Limpiar Finalizados
          </button>
        </div>
      ...
      <div className="flex-1 overflow-x-auto flex flex-col md:flex-row gap-6 p-4 md:p-8 bg-[#0a0a0a]">
        ...
        <div className="w-full md:w-[380px] flex-shrink-0 flex flex-col gap-6">
          <h3 className="text-xs font-black text-stone-500 uppercase tracking-widest px-4">
            Pendientes / Nuevos ({orders.filter((o) => o.status === OrderStatus.PENDING).length})
          </h3>
          <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
            {orders
              ...
              .map((o) => (
                <div
                  key={o.id}
                  className={`p-8 rounded-[2.5rem] border-2 shadow-2xl transition-all hover:scale-[1.02] ${getStatusColor(o.status)}`}
                >
                  <div className="flex justify-between items-start mb-6">
                    <span className="text-xs font-black text-stone-500 tracking-widest">{o.orderNumber}</span>
                    <span className="text-[10px] bg-stone-800 px-3 py-1 rounded-full font-black uppercase">
                      {o.paymentMethod}
                    </span>
                  </div>
                    ...
                    {o.items.map((item) => (
                      <div key={item.id} className="flex flex-col text-sm">
                        <div className="flex justify-between items-center">
                          <span className="font-black text-orange-500 uppercase tracking-tighter">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 bg-orange-600 hover:bg-orange-500 py-4 rounded-2xl font-black transition-all text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-orange-900/20"
                    >
                      PREPARAR
                    </button>
                  </div>
        ...
        <div className="w-full md:w-[380px] flex-shrink-0 flex flex-col gap-6">
          <h3 className="text-xs font-black text-orange-500 uppercase tracking-widest px-4">
            En Preparación ({orders.filter((o) => o.status === OrderStatus.PREPARING).length})
          </h3>
          <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
            {orders
              ...
              .map((o) => (
                <div
                  key={o.id}
                  className={`p-8 rounded-[2.5rem] border-2 shadow-2xl transition-all ${getStatusColor(o.status)}`}
                >
                  <div className="flex justify-between items-start mb-6">
                  ...
                  <button
                    onClick={() => updateStatus(o.id, OrderStatus.READY)}
                    className="w-full bg-orange-600 hover:bg-orange-500 py-4 rounded-2xl font-black transition-all text-[10px] uppercase tracking-[0.2em]"
                  >
                    MARCAR LISTO
                  </button>
                </div>
        ...
        {/* Columna: Listos */}
        <div className="w-full md:w-[380px] flex-shrink-0 flex flex-col gap-6">
          <h3 className="text-xs font-black text-green-500 uppercase tracking-widest px-4">
            Listos / Despacho ({orders.filter((o) => o.status === OrderStatus.READY).length})
          </h3>
          <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
            {orders
              ...
              .map((o) => (
                ...
                >
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => updateStatus(o.id, OrderStatus.COMPLETED)}
                      className="flex-1 bg-green-600 hover:bg-green-500 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all"
                    >
                      ENTREGADO
                    </button>
                    <button
                      onClick={() => updateStatus(o.id, OrderStatus.DELIVERING)}
                      className="flex-1 bg-stone-800 hover:bg-stone-700 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border border-stone-700"
                    >
                      REPARTO
                    </button>
                  </div>
