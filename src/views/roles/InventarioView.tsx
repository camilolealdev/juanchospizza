...
const InventarioView: React.FC = () => {
  ...
  const [addForm, setAddForm] = useState({
    ...
    unidad: '', costoUnitario: '', lote: '', fechaVencimiento: '',
  });
  ...
  const handleAddSubmit = async () => {
    if (!addForm.nombre.trim()) { showToast('Completa el nombre'); return; }
    try {
      ...
      setAddForm({ nombre: '', categoria: '', stockActual: '', stockMinimo: '', unidad: '', costoUnitario: '', lote: '', fechaVencimiento: '' });
      showToast('Insumo agregado');
      loadAll();
    } catch (e) {
      showToast(`Error agregando insumo: ${e instanceof Error ? e.message : 'error desconocido'}`);
    }
  };

  const handleMoveSubmit = async () => {
    if (!moveTarget || !moveForm.cantidad || !moveForm.motivo.trim()) { showToast('Completa cantidad y motivo'); return; }
    try {
    ...
    <div className="p-8 md:p-12 space-y-16 pb-40 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        {[
          ...
          { label: 'Movimientos', value: `${movements.length}`, icon: 'right-left', color: 'text-purple-500', trend: 'últimos 50' },
        ].map((stat, i) => (
        ...
        <div className="bg-stone-900/30 rounded-[2.5rem] border border-stone-800/50 shadow-2xl">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {inventory.length === 0 && (
                  <tr><td colSpan={10} className="p-10 text-center text-stone-600 font-bold text-xs uppercase tracking-widest">Sin insumos registrados</td></tr>
                )}
                {inventory.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id} className={`border-b border-stone-800/40 transition-all hover:bg-stone-800/20 ${idx % 2 === 0 ? 'bg-stone-900/10' : ''}`}>
                      ...
                      <td className="p-6 text-stone-400 text-[11px]">{item.fechaVencimiento ? new Date(item.fechaVencimiento).toLocaleDateString('es-CO') : '—'}</td>
      ...
      <section className="space-y-8">
        <div className="border-b border-white/5 pb-8">
          ...
          <p className="text-stone-600 text-xs mt-2 uppercase tracking-[0.4em] font-bold">Entradas, salidas y ajustes</p>
        </div>

        <div className="bg-stone-900/30 rounded-[2.5rem] border border-stone-800/50 divide-y divide-stone-800/40 shadow-2xl">
          {movements.length === 0 && (
            <p className="p-10 text-center text-stone-600 font-bold text-xs uppercase tracking-widest">Sin movimientos registrados</p>
          )}
          {movements.map((mov) => {
            ...
            return (
              <div key={mov.id} className="flex items-center gap-6 p-6 px-8 hover:bg-stone-800/20 transition-all">
                ...
                <div className="text-right">
                  <p className={`font-black text-lg ${mov.tipo.toLowerCase() === 'entrada' ? 'text-green-400' : 'text-stone-200'}`}>
                    {mov.tipo.toLowerCase() === 'entrada' ? '+' : '-'}{mov.cantidad}
                  </p>
                </div>
                <div className="text-right w-40">
                  <p className="text-[11px] text-stone-400 font-medium">{new Date(mov.creado).toLocaleString('es-CO')}</p>
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {recipes.length === 0 && (
            <p className="col-span-full text-center text-stone-600 font-bold text-xs uppercase tracking-widest py-12">Sin recetas registradas</p>
          )}
          {recipes.map((receta) => (
              ...
              <div className="p-8 space-y-4">
                {receta.ingredientes.map((ing) => (
                  <div key={ing.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <span className="text-[11px] text-stone-500 font-mono">{ing.cantidad} {ing.unidad}</span>
      ...
      {showMoveModal && moveTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-stone-900 border border-stone-700 rounded-[2rem] p-10 w-full max-w-lg mx-4 shadow-2xl z-10">
            ...
            <p className="text-stone-500 text-sm mb-8">Stock actual: {moveTarget.stockActual} {moveTarget.unidad}</p>
            <div className="space-y-5">
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] font-black text-stone-500 mb-2 block">Tipo</label>
              ...
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] font-black text-stone-500 mb-2 block">Cantidad</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-[10px] uppercase tracking-[0.3em] font-black text-stone-500 mb-2 block">Motivo</label>
                <input
                  type="text"
