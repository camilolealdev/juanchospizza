...
const ClientesView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((rows) => {
        if (!cancelled) setClientOrders(rows.map(summarizeOrder));
      })
      .catch(() => {
        if (!cancelled) setClientOrdersError('No se pudo cargar el historial de compras.');
      })
      .finally(() => {
  ...
  const kpis = useMemo(
    () => [
      ...
      {
        label: 'Clientes VIP',
        value: String(clients.filter((c) => c.vip).length),
        icon: 'crown',
      ...
      },
      {
        ...
        value: String(clients.filter((c) => c.estado === 'inactivo').length),
        icon: 'user-clock',
        color: 'text-red-400',
      ...
      },
      {
        ...
        value: formatCurrency(
          clients.length ? Math.round(clients.reduce((s, c) => s + c.totalGastado, 0) / clients.length) : 0
        ),
        icon: 'chart-line',
    ...
    () => [
      {
        ...
        count: clients.filter((c) => c.frecuenciaCompra >= 3).length,
        desc: '> 3 compras en el último mes',
        icon: 'rocket',
      ...
      },
      {
        id: 'alto-gasto',
        label: 'Alto Gasto',
        count: clients.filter((c) => c.totalCompras > 0 && c.totalGastado / c.totalCompras > 150000).length,
        desc: 'Ticket promedio > $150,000',
        icon: 'gem',
      ...
      },
      {
        id: 'en-riesgo',
        label: 'En Riesgo',
        count: clients.filter((c) => daysSince(c.ultimaCompra) >= 45).length,
        desc: 'Sin compras en 45+ días',
      ...
      },
      {
        id: 'nuevos',
        label: 'Nuevos',
        count: clients.filter((c) => daysSince(c.creado) <= 30).length,
        desc: 'Primera compra < 30 días',
        icon: 'star',
        color: 'text-sky-400',
  ...
  const filteredClients = useMemo(() => {
    ...
    if (segmentFilter === 'alto-gasto')
      result = result.filter((c) => c.totalCompras > 0 && c.totalGastado / c.totalCompras > 150000);
    ...
    if (search) {
      ...
      result = result.filter(
        (c) =>
          c.nombre.toLowerCase().includes(q) || c.telefono.includes(q) || (c.email && c.email.toLowerCase().includes(q))
      );
    }
  ...
  const renderKPIs = () => (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
      {kpis.map((k) => (
        <div
          key={k.label}
          className={`${k.bg} backdrop-blur-xl rounded-[2rem] border p-6 md:p-8 flex items-center gap-5`}
        >
  ...
  const renderSearchBar = () => (
    <div className="flex flex-col lg:flex-row gap-4">
      <div className="flex-1 flex items-center bg-stone-900/60 backdrop-blur-xl rounded-[2rem] border border-white/5 px-6 py-4 focus-within:border-orange-600/50 transition-all">
        ...
        />
        {search && (
          <button onClick={() => setSearch('')} className="text-stone-600 hover:text-white ml-2">
  ...
  const renderSegmentCard = (seg: (typeof segments)[0]) => (
    ...
    >
      <div className="flex items-center gap-4 mb-4">
        <div
          className={`w-12 h-12 rounded-2xl bg-stone-800 flex items-center justify-center group-hover:scale-110 transition-transform`}
        >
  ...
  const renderClientCard = (client: Client) => (
    ...
    >
      <div className="flex items-start gap-5 mb-6">
        <div
          className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${NIVEL_COLORS[client.nivel] || 'from-stone-600 to-stone-400'} flex items-center justify-center text-white font-black text-lg shrink-0 shadow-lg`}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap mb-1">
            ...
            {client.vip && (
              <span className="bg-yellow-600/20 text-yellow-400 text-[8px] font-black px-3 py-1.5 rounded-full border border-yellow-500/30 uppercase tracking-wider">
                <i className="fas fa-crown mr-1"></i>VIP
              </span>
            )}
          </div>
          <p className="text-stone-500 text-xs mb-1">
            <i className="fas fa-phone-alt mr-2 text-stone-600"></i>
            {client.telefono}
          </p>
          {client.email && (
            <p className="text-stone-500 text-xs truncate">
              <i className="fas fa-envelope mr-2 text-stone-600"></i>
              {client.email}
            </p>
          )}
        </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5">
        <span
          className={`text-[8px] font-black uppercase px-4 py-2 rounded-full border ${NIVEL_BG[client.nivel] || NIVEL_BG.Bronce}`}
        >
          {client.nivel}
        </span>
        <div className="flex items-center gap-2">
          <span
            className={`w-2.5 h-2.5 rounded-full ${client.estado === 'activo' ? 'bg-green-500' : 'bg-red-500'} shadow-lg`}
          ></span>
  ...
  const handleDeleteClient = async (client: Client) => {
    ...
    try {
      ...
      // El backend responde 409 con un mensaje ya listo para mostrar si el
      // cliente tiene pedidos asociados (FK protegida a propósito, ver
      // server/routes/clients.js) -- se preserva el historial en vez de
      // borrarlo en cascada.
      showToast(e instanceof Error ? e.message : 'No se pudo eliminar el cliente.');
    }
  };
  ...
  const renderNewClientModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-stone-900/95 backdrop-blur-2xl rounded-[3rem] border border-white/10 max-w-lg w-full p-8 shadow-2xl shadow-black/50"
        ...
        >
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              onClick={() => setShowNewForm(false)}
              className="w-12 h-12 rounded-2xl bg-stone-800 flex items-center justify-center text-stone-400 hover:text-white transition-all"
            >
              <i className="fas fa-times"></i>
            </button>
          </div>
          <form onSubmit={handleNewClientSubmit} className="space-y-5">
            <div>
              <label className="text-[9px] font-black text-stone-500 uppercase tracking-[0.2em] block mb-2">
                Nombre *
              </label>
              <input
                type="text"
                ...
                className="w-full bg-stone-800/50 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm focus:border-orange-600/50 transition-all outline-none"
            ...
            <button
              type="submit"
              disabled={!isValid || isSavingNewClient}
              className="w-full bg-orange-600 hover:bg-orange-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-black py-5 rounded-2xl text-[10px] uppercase tracking-widest transition-all shadow-lg"
            >
              <i className="fas fa-plus mr-2"></i>
              {isSavingNewClient ? 'Guardando...' : 'Registrar Cliente'}
            </button>
          </form>
        </div>
      </div>
  ...
  const renderEditClientModal = () => {
    ...
    return (
      <div
        className="fixed inset-0 z-[210] bg-stone-950/80 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <form onSubmit={handleEditClientSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditClient((p) => ({ ...p, nombre: e.target.value }))}
                className="w-full bg-stone-800/50 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm focus:border-orange-600/50 transition-all outline-none"
                placeholder="Nombre completo"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditClient((p) => ({ ...p, telefono: e.target.value }))}
                className="w-full bg-stone-800/50 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm focus:border-orange-600/50 transition-all outline-none"
                placeholder="+57 300 000 0000"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditClient((p) => ({ ...p, direccion: e.target.value }))}
                className="w-full bg-stone-800/50 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm focus:border-orange-600/50 transition-all outline-none"
                placeholder="Dirección"
              />
            </div>
  ...
  const renderModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-stone-900/95 backdrop-blur-2xl rounded-[3rem] border border-white/10 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl shadow-black/50"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="sticky top-0 bg-stone-900/95 backdrop-blur-xl z-10 flex items-center justify-between p-6 md:p-8 border-b border-white/5 rounded-t-[3rem]">
            <div className="flex items-center gap-5">
              <div
                className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${NIVEL_COLORS[c.nivel] || 'from-stone-600 to-stone-400'} flex items-center justify-center text-white font-black text-xl shadow-lg`}
              >
                {getInitial(c.nombre)}
              </div>
              <div>
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-[8px] font-black px-3 py-1.5 rounded-full border ${NIVEL_BG[c.nivel] || NIVEL_BG.Bronce}`}
                  >
                    {c.nivel}
                  </span>
                  <span
                    className={`w-2 h-2 rounded-full ${c.estado === 'activo' ? 'bg-green-500' : 'bg-red-500'}`}
                  ></span>
                </div>
              </div>
            </div>
            <button
              onClick={() => setSelectedClient(null)}
              className="w-12 h-12 rounded-2xl bg-stone-800 flex items-center justify-center text-stone-400 hover:text-white hover:bg-stone-700 transition-all"
            >
              <i className="fas fa-times"></i>
            </button>
          </div>
          ...
          <div className="p-6 md:p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <h4 className="text-[9px] font-black text-stone-500 uppercase tracking-[0.2em]">
                  Información de Contacto
                </h4>
                <div className="bg-stone-800/30 rounded-2xl p-5 space-y-3 border border-white/5">
                  <p className="text-white text-sm">
                    <i className="fas fa-phone-alt mr-3 text-stone-600 w-4"></i>
                    {c.telefono}
                  </p>
                  {c.email && (
                    <p className="text-white text-sm">
                      <i className="fas fa-envelope mr-3 text-stone-600 w-4"></i>
                      {c.email}
                    </p>
                  )}
                  {c.direccion && (
                    <p className="text-white text-sm">
                      <i className="fas fa-map-marker-alt mr-3 text-stone-600 w-4"></i>
                      {c.direccion}
                    </p>
                  )}
                  <p className="text-white text-sm">
                    <i className="fas fa-calendar mr-3 text-stone-600 w-4"></i>Cliente desde {formatDate(c.creado)}
                  </p>
                  {c.cumpleanos && (
                    <p className="text-white text-sm">
                      <i className="fas fa-birthday-cake mr-3 text-stone-600 w-4"></i>
                      {formatDate(c.cumpleanos)}
                    </p>
                  )}
                </div>
              </div>
                  ...
                  {c.tags.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {c.tags.map((tag) => (
                        <span
                          key={tag}
                          className="bg-orange-600/15 text-orange-400 text-[9px] font-black px-4 py-2 rounded-full border border-orange-500/20 uppercase tracking-wider"
                        >
                          <i className="fas fa-tag mr-1.5 opacity-60"></i>
                          {tag}
                        </span>
                      ))}
                    </div>
              ...
              <div className="bg-stone-800/30 rounded-2xl border border-white/5 overflow-hidden">
                ...
                {!clientOrdersLoading &&
                  ...
                  clientOrders.map((o, i) => (
                    <div
                      key={o.id}
                      className={`flex items-center justify-between p-4 md:p-5 ${i < clientOrders.length - 1 ? 'border-b border-white/5' : ''} hover:bg-stone-800/50 transition-colors`}
                    >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 pt-2">
              ...
              <button
                onClick={() => showToast('Asignación de cupones todavía no está implementada.')}
                title="Función en desarrollo -- no hay sistema de cupones en el backend todavía"
                className="flex items-center justify-center gap-3 bg-stone-800 text-stone-400 border border-white/5 font-black py-5 rounded-2xl text-[10px] uppercase tracking-widest transition-all shadow-lg cursor-not-allowed"
              >
              ...
              >
                ...
                {c.vip ? 'Quitar VIP' : 'Marcar VIP'}
              </button>
              <button
                onClick={() => handleDeleteClient(c)}
                className="flex items-center justify-center gap-3 bg-stone-800 text-stone-300 hover:bg-red-600/20 hover:text-red-400 hover:border-red-500/30 border border-white/5 font-black py-5 rounded-2xl text-[10px] uppercase tracking-widest transition-all shadow-lg"
              >
                <i className="fas fa-trash text-lg"></i>Eliminar
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="p-6 md:p-10 space-y-8 pb-40">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-stone-500 mt-3 max-w-xl">
            Gestión inteligente de la base de clientes GastroPro. Segmenta, analiza y fideliza.
          </p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div>
            <h3 className="text-[9px] font-black text-stone-500 uppercase tracking-[0.2em] mb-5">
              Segmentación Rápida
            </h3>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{segments.map(renderSegmentCard)}</div>
          </div>
