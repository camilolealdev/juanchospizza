  ...
  return (
    <div className="p-8 md:p-12 space-y-10 pb-40 animate-fade-in">
      ...
      {loading ? (
        ...
        <p className="text-center text-stone-600 font-bold text-xs uppercase tracking-widest py-20">No hay reseñas en esta categoría</p>
      ) : (
          ...
          {reviews.map(r => (
            <div key={r.id} className="bg-stone-900/40 p-8 rounded-[2.5rem] border border-white/5 space-y-4">
              <div className="flex justify-between items-start">
                ...
                <span className="text-[10px] text-stone-600 uppercase tracking-widest font-bold">{new Date(r.createdAt).toLocaleDateString('es-CO')}</span>
              </div>
              {r.comment && <p className="text-stone-300 text-sm italic">&ldquo;{r.comment}&rdquo;</p>}
              <p className="text-stone-500 text-xs">{r.clientName || 'Cliente'} {r.clientPhone ? `— ${r.clientPhone}` : ''}</p>
              {tab === 'pending' && (
                <div className="flex gap-3 pt-2">
                  <button onClick={() => updateStatus(r.id, 'approved')} className="flex-1 bg-green-600 hover:bg-green-500 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Aprobar</button>
                  <button onClick={() => updateStatus(r.id, 'rejected')} className="flex-1 bg-red-600 hover:bg-red-500 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Rechazar</button>
                </div>
              )}
            </div>
          ))}
