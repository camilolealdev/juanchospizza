...
const GASTRO_MODULES: GastroModule[] = [
  'dashboard', 'menu', 'inventario', 'clientes', 'fidelizacion', 'campanas',
  'finanzas', 'reportes', 'reviews', 'pagos', 'notificaciones', 'empleados',
  'turnos', 'mesas', 'caja', 'comandas', 'compras', 'facturacion', 'digiturno', 'derechos',
];
...
const App: React.FC = () => {
  ...
  const renderGastroModule = () => {
    switch (gastroModule) {
      ...
      case 'turnos': return <TurnosView locationId={selectedLocation} />;
      case 'mesas': return <MesasView locationId={selectedLocation} />;
      case 'caja': return <CajaView locationId={selectedLocation} />;
      case 'comandas': return <ComandasView locationId={selectedLocation} />;
      case 'compras': return <ComprasView locationId={selectedLocation} />;
      case 'facturacion': return <InvoicesView locationId={selectedLocation} />;
      case 'digiturno': return <DigiturnoView locationId={selectedLocation} />;
      case 'derechos': return <DerechosView />;
      default: return <GastroProDashboard locationId={selectedLocation} />;
    }
  };

  const isStaffAdmin = isAuthenticated &&
    (role === UserRole.ADMIN || role === UserRole.OPERATOR ||
     role === UserRole.REPARTIDOR || role === UserRole.MARKETING);
