...
const MenuDigital: React.FC<{ onClose?: () => void; variant?: 'overlay' | 'section' }> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (isOverlay) document.body.style.overflow = '';
    };
      ...
      <div className="sticky top-0 z-20 bg-[#F4EFEA]/95 backdrop-blur-xl border-b border-[#8B572A]/10">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between">
          ...
          >
            ...
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-[#F9DC5C] text-[#1A1A1A] flex items-center justify-center text-[10px] font-black shadow-lg">
                {cartCount}
              </span>
            )}
          </button>
          ...
          >
            ...
            >
              ...
              <div className="p-6 border-b border-[#8B572A]/10 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-black text-[#1A1A1A]" style={{ fontFamily: "'Bitter', serif" }}>
                    Tu Pedido
                  </h2>
