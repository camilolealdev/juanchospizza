...
const PizzaConfigurator: React.FC = () => {
  ...
  const sizeInfo = useMemo(
    () => PIZZA_SIZES.find((s) => s.id === selectedSize) || null,
    [selectedSize]
  );
          ...
          {[1, 2, 3].map((step) => (
            <React.Fragment key={step}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-heading tracking-wider uppercase transition-colors duration-300 hidden sm:block ${
                    ...
                    {selectedFlavors.size > 0 && (
                      <div className="flex flex-wrap justify-center gap-1.5 mt-3">
                        {flavorNames.map((name) => (
                          <span key={name} className="px-2.5 py-1 bg-queso/15 text-queso text-xs font-heading tracking-wider rounded-full">
                            {name}
                          </span>
                        ))}
                      </div>
                    )}
                    <div className="font-heading text-3xl text-queso mt-4">
          ...
          <div className="lg:col-span-3 order-2 space-y-8">
            ...
            <div className={`transition-all duration-300 ${currentStep >= 1 ? 'opacity-100' : 'opacity-40'}`}>
              <div className="flex items-center gap-3 mb-5">
                <span className={`w-10 h-10 rounded-xl flex items-center justify-center font-heading text-sm transition-all duration-300 ${
                ...
                {PIZZA_SIZES.map((size) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-xs text-crema/40 mt-1">
                        {size.portions} porciones
                      </div>
                      <div className="text-[10px] text-crema/25 mt-0.5">
                        Hasta {size.maxFlavors} sabor{size.maxFlavors > 1 ? 'es' : ''}
                      </div>
                      <div className={`font-heading text-lg mt-2 transition-colors ${isSelected ? 'text-queso' : 'text-tomato group-hover:text-queso'}`}>
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          className="sr-only"
                        />
                        <span className={`text-xl mt-0.5 transition-transform duration-300 ${checked ? 'scale-125' : ''}`}>
                          {FLAVOR_ICONS[flavor.id] || '🍕'}
                        </span>
                        <div className="min-w-0 flex-1">
                          <div className={`text-sm font-heading tracking-wider transition-colors ${checked ? 'text-queso' : 'text-crema/80'}`}>
                            {flavor.name}
                          </div>
                          <div className="text-[11px] text-crema/35 leading-tight mt-0.5">
                            {flavor.ingredients}
                          </div>
                        </div>
                        {checked && (
              ...
              <div className="animate-fade-in">
                ...
                <div className="bg-crema/5 backdrop-blur-sm rounded-2xl border border-crema/10 p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="font-heading text-lg text-queso">
                      {formatPrice(sizeInfo.price)}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {flavorNames.map((name) => (
                      <span key={name} className="px-2.5 py-1 bg-crema/10 text-crema/60 text-xs font-heading tracking-wider rounded-full">
                        {name}
                      </span>
                    ))}
                  </div>
                  <div className="border-t border-crema/10 pt-3 flex items-center justify-between">
                    ...
                    <span className="font-heading text-2xl text-queso">
                      {formatPrice(sizeInfo.price)}
                    </span>
                  </div>
                </div>
