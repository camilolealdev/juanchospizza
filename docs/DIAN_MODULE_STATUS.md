...
>
> **Alcance del estado:** la estructura y los componentes de firma/adaptadores están implementados en código. La emisión legal end-to-end sigue bloqueada por datos reales del emisor, certificado, software DIAN, credenciales y validación en habilitación. Este documento reemplaza las afirmaciones de "guía por implementar" de versiones anteriores.
...
| Estructura Firma Digital               | ✅ Implementada                                       | `server/services/dianSigner.js`                 |
...
| **Firma digital con certificado real** | 🟠 Requiere certificado/datos reales                  | Ver abajo                                       |
| **Conexión proveedor DIAN**            | 🟠 Adaptadores implementados; falta activar y validar | `server/services/dianProvider.js` + ruta `send` |
| **Certificado digital**                | 🟠 Externo, aún no provisionado                       | `.p12`/`.pfx` fuera del repo                    |
| **CUFE real**                          | 🟠 Validación en habilitación pendiente               | DIAN/proveedor tecnológico                      |
...
### Preparación de envío a DIAN (sin activación externa)

- `POST /api/invoices/:id/send` — prepara el payload y el flujo interno; la activación del adapter y el envío legal requieren configuración externa
- Actualiza estado a `sent` en el flujo actual
- Devuelve instrucciones para completar la activación
...
### 4. Obtener y configurar certificado digital
...
3. Verificar el flujo XAdES-EPES existente con el certificado real y ajustar solo lo que falle en habilitación
4. Validar `calcularCUFE()` con una factura de prueba y la respuesta oficial del proveedor

### 5. Elegir y activar proveedor tecnológico
...
2. Configurar credenciales/endpoints del adapter seleccionado en `server/services/dianProvider.js`
3. Conectar/activar `sendToDataico()` o `sendToMuisca()` desde `POST /api/invoices/:id/send` y probarlo en habilitación
...
2. 🟠 Activar firma con certificado real y validarla en habilitación (ver `dianSigner.js`)
3. 🟠 Activar el adapter elegido desde `POST /api/invoices/:id/send`
4. 🟠 Validar CUFE y respuesta contra DIAN/proveedor en habilitación
...
| `server/services/dianSigner.js`    | Implementación de firma XAdES-EPES + cálculo CUFE, pendiente de validar con certificado real |
