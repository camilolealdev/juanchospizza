...
**Método:** extracción AST real por agente (34 tablas, 32 routers, 20 vistas, 130 métodos API) + verificación de gaps conocidos

---

## 📊 Resumen Ejecutivo

**Backend tiene TODO el CRUD de las 34 tablas cableado** (incluidos los 3 verbos faltantes agregados el 05-08). **El frontend cubre los 19 módulos.** La deuda real no es "falta lo que no existe" — es **calidad de lo existente**: 18 routers con PUT de lista fija (riesgo NULL-overwrite), 0 paginación real en listas grandes, ~25 rutas sin un solo test, bundle con 363KB de recharts (96KB gzip). **Los 8MB de imágenes/video ya no existen: -6.82MB liberados el 05-08 (ver §7).**
...
| `recipes`            | id, nombre, porciones, instrucciones                                                                                             | ✅     | ⚠️ `recipe_ingredients` sin FK a `recipes`                                                                                                                |
| `recipe_ingredients` | id, nombre, cantidad, unidad, costo                                                                                              | ✅     | huérfana de recipes — ver gap R5                                                                                                                          |
...
| `comandas`           | id, status, notes, total                                                                                      | ✅     | ⚠️ NO tiene `clientId`, `locationId` ni `tableId` → conciliación caja/turno "mal" (ver AUDIT_07-30)                  |
...
| `cash_register`      | id, difference, status, notes                                                                                 | ✅     | ⚠️ **4 cols únicamente** — sin apertura/cierre por turno real (diferencias decorativas)                              |
| `shifts`             | id, status, difference, notas, openedBy, locationId, openedAt, closedAt                                       | ✅     | —                                                                                                                    |
| `tips`               | id, amount, method                                                                                            | ✅     | sin `shiftId`/`locationId` → tips no concilian con caja                                                              |
...
**🟡 GAP DB medio:** `comandas` sin `locationId`/`tableId`/`clientId` — la conciliación de caja/turno documentada en AUDIT_07-30 como "activamente mal" sigue pendiente.
...
| Notificaciones  | notifications.js | ✅ status                            | ✅ test-email/webhook             | —                | —              | —                              | ✅        | ✅                               |
...
- **PUT con lista fija de columnas (riesgo NULL-overwrite):** 18 routers (`campaigns, categories, clients, comandas, digiturno, employees, finance, ingredients, inventory, invoices, loyalty, menuExtras, orders, pizzaSizes, procurement, products, recipes, tables`) — el PUT arma el UPDATE con whitelist fija; si el frontend no envía un campo, se escribe NULL. **Deuda B-7 de BREACHES_08-04, sigue abierta.**
...
**Solo 5 archivos de test cubren rutas:** `consent`, `digiturno`, `orders`, `payments`, `print`.

**🔴 Sin NINGÚN test (25+ routers):** auth, products, categories, ingredients, pizzaSizes, campaigns, clients, inventory, recipes, finance, loyalty, menuExtras, reviews, menu, tables, shifts, comandas, cashRegister, procurement, invoices, qrMenu, stats, notifications, push, misc, employees, menus.
...
**🔴 El 60% de la API (rutas de dinero/CRM) no tiene red de seguridad.** El flujo de pagos SÍ está cubierto (14 tests webhooks), pero CRUD de productos/clientes/inventario/finanzas NO.
...
**🟡 Medios:** 4. **recharts = 363KB (96KB gzip)** — `BarChart-BuJ3kyop.js` entra solo para el dashboard admin (lazy ✅) pero ReportesView/FinanzasView lo comparten sin carga fina. (B-5 de BREACHES, pendiente) 5. **`index.js` = 245KB (70KB gzip)** — bundle principal: MenuDigital/CartSection/PizzaBuilder importados EAGER en App.tsx (necesarios para la landing, correcto) + framer-motion en el path crítico. 6. **Google Fonts + Font Awesome por CDN** — 2 requests render-blocking terceros.
...
**Prioridad de fix → ✅ COMPLETADA (05-08):** ahorro real **6.82 MB (-80.3%)** en estos 3 assets (medido en `npm run build`). Siguiente pendiente de performance: **recharts (363KB/96KB gzip)** con lazy fino por vista (índice #9).
...
| 5   | 🟠        | Estrategia PUT lista fija (18 routers) — whitelist dinámica con `COALESCE` o full-entity PUT                                                                                                                                   | 18 routers                                   | 1d       |
| 6   | 🟠        | Tests de rutas sin cobertura (auth, clients, products, inventory, finance, employees…)                                                                                                                                         | `server/tests/`                              | 2-3d     |
| 7   | 🟠        | Paginación real en orders/clients/inventory/finance (hoy LIMIT hardcodeado 2000/100/50)                                                                                                                                        | routers + api.ts                             | 1d       |
| 8   | 🟠        | Scheduler de campañas `scheduled` (cron mínimo o bandera de estado)                                                                                                                                                            | `server/`                                    | 0.5d     |
| 9   | 🟡        | Lazy fino de recharts por vista (96KB gzip)                                                                                                                                                                                    | ReportesView/FinanzasView/GastroProDashboard | 1h       |
...
**Conclusión:** No hay "sistema fantasma" ni módulos prometidos que no existan — **todo el CRUD de las 34 tablas y los 19 módulos está implementado, y los gaps 🔴 de seguridad/deploy/perf (#1 assets -6.82MB, #2 sede en tables, #3 processed_webhooks) quedaron resueltos el 05-08**. Las brechas reales son: **~25 routers sin tests, 1 hallazgo de performance menor (recharts 96KB gzip), y 4 automatizaciones prometidas que no se disparan solas (campañas, reconciliación, inventario por receta, monitoreo n8n).**

_Informe generado por auditoría multi-agente el 2026-08-05 — actualizado el mismo día con la resolución de los gaps #1 (assets, -6.82MB), #2 (sede en tables), #3 (processed_webhooks) y #4 (verbos CRUD)._
